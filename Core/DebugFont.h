...
void gDrawDebugFontChar(char inChar, const Frame<T_Pixel>& inFrame, const ivec2& inOffset, const T& inColor)
{
	...
	for (int x = 0; x < 3; x++)
	{
		if (debug_char[inChar-f_start] & (1 << p))
			inFrame.Set(inOffset.x + x, inOffset.y + y, inColor);
		p_count++;
	}
}
