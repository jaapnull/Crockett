...
public:
	...
	typedef void				(AssignFunction)(void*, const void*);					// CopyFunction is used to copy one object onto the other
	...
	template <typename T>
	static void* cAssignInstance(void* inDest, const void* inSource)
	{
		*((T*)inDest) = *((T*inSource));
		return new_object;
	}
...
public:
	...
	void			GetInternalType(FullType& outInternalType) const	{ gAssert(!mModifiers.IsEmpty()); outInternalType = *this; outInternalType.mModifiers.Pop(); }
...
class ReflectionHost : public Singleton<ReflectionHost>
{
public:
	ReflectionHost() {}
	template<typename T>
...
private:
	...
	stdext::hash_map<ClassName, ClassReflectionInfo>	mItems;										// a map from id to instance function		
};
