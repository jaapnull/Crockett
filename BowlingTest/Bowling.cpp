...
There are two perfect bowlers, standing in front of a row of pins (with gaps)
Each bowler gets to either know 1 pin down, or knock 2 adjacent pins down
Given a starting setup of pins, who wins, assuming perfect play

A very interesting problem that was given to me on a forum somewhere; has a lot of really cool stuff like
deep state reduction
non-trivial state transfer functions
hashing and other speed optimizations
**/
...
// State describing group of adjacent bowlign pins
struct State
{
	// stored as N groups of 1...9 bowling pins
...
// Hash functor to trampoline our own hash function
template<> struct std::hash<State>
{
	std::size_t operator()(const State& k) const { return k.GetHash(); }
};

std::unordered_map<State, bool> results;

// Create all recuction sets (state transforms) for a given state
...
// generate state from user TTY input
...
// Evaluate if a state is a winning or losing state, caching already found states
